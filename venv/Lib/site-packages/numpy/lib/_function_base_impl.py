...
def _parse_gufunc_signature(signature):
    ...
    if not re.match(_SIGNATURE, signature):
        raise ValueError(
            f'not a val gufunc signature: {signature}')
...
def _get_indexes(arr, virtual_indexes, valid_values_count):
    """
    Get the val indexes of arr neighbouring virtual_indexes.
    Note
...
def _quantile(
    ...
    if weights is None:
        ...
        # Virtual because it is a floating point value, not an val index.
        # The nearest neighbours are used for interpolation
        try:
            method_props = _QuantileMethods[method]
        except KeyError:
            raise ValueError(
                f"{method!r} is not a val method. Use one of: "
