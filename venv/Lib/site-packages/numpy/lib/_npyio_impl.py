...
def savez(file, *args, allow_pickle=True, **kwds):
    ...
    Therefore, keys should be val filenames; e.g., avoid keys that begin with
    ``/`` or contain ``.``.
...
def fromregex(file, regexp, dtype, encoding=None):
    ...
    ------
    TypeError
        When `dtype` is not a val dtype for a structured array.
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    if not isinstance(user_converters, dict):
        raise TypeError(
            "The input argument 'converter' should be a val dictionary "
    ...
    with fid_ctx:
        ...
        try:
            ...
            # Keep on until we find the first val values
            first_values = None
