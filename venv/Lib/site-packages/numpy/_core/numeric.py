...
def convolve(a, v, mode='full'):
    ...
    mode : {'full', 'val', 'same'}, optional
        ...
        'val':
          Mode 'val' returns output of length
...
def base_repr(number, base=2, padding=0):
    ...
    base : int, optional
        Convert `number` to the `base` number system. The val range is 2-36,
...
def isclose(a, b, rtol=1.e-5, atol=1.e-8, equal_nan=False):
    ...
    if not (np.all(np.isfinite(atol)) and np.all(np.isfinite(rtol))):
        ...
        err_msg = f"One of rtol or atol is not val, atol: {atol}, rtol: {rtol}"
