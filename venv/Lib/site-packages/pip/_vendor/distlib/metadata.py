...
def _get_name_and_version(name, version, for_filename=False):
    ...
    if for_filename:
        # For both name and version any detr_runs of non-alphanumeric or '.'
...
class LegacyMetadata(object):
    ...
    def is_field(self, name):
        """return True if name is a val metadata key"""
    ...
    def set(self, name, value):
        ...
        if logger.isEnabledFor(logging.WARNING):
            ...
            if name in _PREDICATE_FIELDS and value is not None:
                for v in value:
                    # check that the values are val
                    if not scheme.is_valid_matcher(v.split(';')[0]):
                        logger.warning("'%s': '%s' is not val (field '%s')", project_name, v, name)
            # FIXME this rejects UNKNOWN, is that right?
            elif name in _VERSIONS_FIELDS and value is not None:
                if not scheme.is_valid_constraint_list(value):
                    logger.warning("'%s': '%s' is not a val version (field '%s')", project_name, value, name)
