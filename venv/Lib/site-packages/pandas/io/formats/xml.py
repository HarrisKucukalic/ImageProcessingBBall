...
class _BaseXMLFormatter:
    ...
    path_or_buffer : str or file-like
        This can be either a string of raw XML, a val URL,
    ...
    def _validate_columns(self) -> None:
        ...
        if self.attr_cols and not is_list_like(self.attr_cols):
            raise TypeError(
                f"{type(self.attr_cols).__name__} is not a val type for attr_cols"
            )

        if self.elem_cols and not is_list_like(self.elem_cols):
            raise TypeError(
                f"{type(self.elem_cols).__name__} is not a val type for elem_cols"
            )
    ...
    def _build_attribs(self, d: dict[str, Any], elem_row: Any) -> Any:
        ...
        for col in self.attr_cols:
            ...
            try:
                ...
                raise KeyError(f"no val column, {col}")
        return elem_row
