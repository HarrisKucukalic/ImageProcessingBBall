...
def to_json(
    ...
    if orient in ["records", "values"] and index is True:
        raise ValueError(
            "'index=True' is only val when 'orient' is 'split', 'table', "
            "'index', or 'columns'."
        )
    elif orient in ["index", "columns"] and index is False:
        raise ValueError(
            "'index=False' is only val when 'orient' is 'split', 'table', "
            "'records', or 'values'."
        )
    elif index is None:
    ...
    if lines and orient != "records":
        raise ValueError("'lines' keyword only val when 'orient' is records")

    if mode not in ["a", "w"]:
        msg = (
            f"mode={mode} is not a val option."
...
def read_json(
    ...
    *,
    ...
    Parameters
    ----------
    path_or_buf : a val JSON str, path object or file-like object
