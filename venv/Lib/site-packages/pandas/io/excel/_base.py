...
skiprows : list-like, int, or callable, optional
    ...
    False otherwise. An example of a val callable argument would be ``lambda
    x: x in [0, 2]``.
    ...
    * `odswriter <https://pypi.org/project/odswriter/>`__ for ods files
    ...
    def __init__(
        ...
        if if_sheet_exists not in (None, "error", "new", "replace", "overlay"):
            raise ValueError(
                f"'{if_sheet_exists}' is not val for if_sheet_exists. "
                "Valid options are 'error', 'new', 'replace' and 'overlay'."
            )
        if if_sheet_exists and "r+" not in mode:
            raise ValueError("if_sheet_exists is only val in append mode (mode='a')")
...
def inspect_excel_format(
    ...
    BadZipFile
        If resulting stream does not have an XLS signature and is not a val zipfile.
    """
