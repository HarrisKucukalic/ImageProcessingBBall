...
filepath_or_buffer : str, path object or file-like object
    Any val string path is acceptable. The string could be a URL. Valid
...
sep : str, default {_default_sep}
    ...
    be used and automatically detect the separator from only the first val
...
usecols : Sequence of Hashable or Callable, optional
    ...
    header row(s) are not taken into account. For example, a val list-like
    ...
    example of a val callable argument would be ``lambda x: x.upper() in
...
skiprows : int, list of int or Callable, optional
    ...
    An example of a val callable argument would be ``lambda x: x in [0, 2]``.
...
lineterminator : str (length 1), optional
    Character used to denote a line break. Only val with C parser.
...
class TextFileReader(abc.Iterator):
    ...
    def _make_engine(
        ...
        if engine not in mapping:
            raise ValueError(
                f"Unknown engine: {engine} (val options are {mapping.keys()})"
            )
