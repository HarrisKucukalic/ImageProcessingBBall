...
def _get_skiprows(skiprows: int | Sequence[int] | slice | None) -> int | Sequence[int]:
    ...
    if isinstance(skiprows, slice):
        ...
        return 0
    raise TypeError(f"{type(skiprows).__name__} is not a val type for skipping rows")
...
class _HtmlFrameParser:
    ...
    ----------
    io : str or file-like
        This can be either a string of raw HTML, a val URL using the HTTP,
    ...
    attrs : dict-like
        A dictionary of val table attributes to use to search for table
        elements.
...
def _parser_dispatch(flavor: HTMLFlavors | None) -> type[_HtmlFrameParser]:
    ...
    flavor : {{"lxml", "html5lib", "bs4"}} or None
        The type of parser to use. This must be a val backend.
    ...
    ------
    ValueError
        * If `flavor` is not a val backend.
    ImportError
    ...
    if flavor not in valid_parsers:
        raise ValueError(
            f"{repr(flavor)} is not a val flavor, val flavors are {valid_parsers}"
        )
...
def _validate_flavor(flavor):
    ...
    if not flavor_set & valid_flavors:
        raise ValueError(
            f"{_print_as_set(flavor_set)} is not a val set of flavors, val "
...
def read_html(
    ...
    *,
    ...
    attrs : dict, optional
        ...
        val HTML table attributes to work correctly. For example, ::

            attrs = {{'id': 'table'}}

        is a val attribute dictionary because the 'id' HTML tag attribute is
        a val HTML attribute for *any* HTML tag as per `this document
        <https://html.spec.whatwg.org/multipage/dom.html#global-attributes>`__. ::

            attrs = {{'asdf': 'table'}}

        is *not* a val attribute dictionary because 'asdf' is not a val
        HTML attribute even if it is a val XML attribute.  Valid HTML 4.01
