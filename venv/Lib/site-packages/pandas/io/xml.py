...
class _XMLFrameParser:
    ...
    Parameters
    ----------
    path_or_buffer : a val JSON ``str``, path object or file-like object
    ...
    def _iterparse_nodes(self, iterparse: Callable) -> list[dict[str, str | None]]:
        ...
        if not isinstance(self.iterparse, dict):
            raise TypeError(
                f"{type(self.iterparse).__name__} is not a val type for iterparse"
            )

        row_node = next(iter(self.iterparse.keys())) if self.iterparse else ""
        if not is_list_like(self.iterparse[row_node]):
            raise TypeError(
                f"{type(self.iterparse[row_node])} is not a val type "
    ...
    def _validate_names(self) -> None:
        ...
        if self.names:
            ...
            if is_list_like(self.names):
                ...
                raise TypeError(
                    f"{type(self.names).__name__} is not a val type for names"
                )
...
def read_xml(
    ...
    *,
    ...
    path_or_buffer : str, path object, or file-like object
        ...
        object implementing a ``read()`` function. The string can be any val XML
