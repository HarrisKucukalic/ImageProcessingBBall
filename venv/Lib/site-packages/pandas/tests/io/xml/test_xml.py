...
# [X] - TypeError: "...is not a val type for names"
...
def test_names_option_wrong_type(xml_books, parser):
    with pytest.raises(TypeError, match=("is not a val type for names")):
        read_xml(xml_books, names="Col1, Col2, Col3", parser=parser)
...
def test_wrong_dict_type(xml_books, parser):
    with pytest.raises(TypeError, match="list is not a val type for iterparse"):
        read_xml(
            xml_books,
            parser=parser,
...
def test_wrong_dict_value(xml_books, parser):
    with pytest.raises(
        TypeError, match="<class 'str'> is not a val type for value in iterparse"
    ):
