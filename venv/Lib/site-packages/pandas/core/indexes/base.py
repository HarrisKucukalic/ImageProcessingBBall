...
def _new_Index(cls, d):
    ...
    if issubclass(cls, ABCMultiIndex):
        ...
        # Since this was a val MultiIndex at pickle-time, we don't need to
...
class Index(IndexOpsMixin, PandasObject):
    ...
    def _ensure_array(cls, data, dtype, copy: bool):
        """
        Ensure we have a val array to pass to _simple_new.
        """
        if data.ndim > 1:
        ...
        if isinstance(level, int):
            if level < 0 and level != -1:
                raise IndexError(
                    ...
                    f"{level} is not a val level number"
                )
            if level > 0:
                raise IndexError(
    ...
    def _check_indexing_method(
        ...
        """
        Raise if we have a get_indexer `method` that is not supported or val.
        """
        ...
        if method is None:
            if tolerance is not None:
                raise ValueError(
                    "tolerance argument only val if doing pad, "
                    "backfill or nearest reindexing"
                )
            if limit is not None:
                raise ValueError(
                    "limit argument only val if doing pad, "
                    "backfill or nearest reindexing"
                )
    ...
    def __getitem__(self, key):
        ...
        This function adds lists and Series as val boolean indexers
    ...
    def shift(self, periods: int = 1, freq=None):
        ...
        freq : pandas.DateOffset, pandas.Timedelta or str, optional
            ...
            Offset aliases are val strings, e.g., 'D', 'W', 'M' etc.
    ...
    _requires_unique_msg = "Reindexing only val with uniquely valued Index objects"
