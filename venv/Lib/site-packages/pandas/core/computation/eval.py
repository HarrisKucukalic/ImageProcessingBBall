...
def _check_engine(engine: str | None) -> str:
    """
    Make sure a val engine is passed.
    ...
    if engine not in ENGINES:
        ...
        raise KeyError(
            f"Invalid engine '{engine}' passed, val engines are {valid_engines}"
        )
...
def _check_parser(parser: str):
    """
    Make sure a val parser is passed.
    ...
    if parser not in PARSERS:
        raise KeyError(
            f"Invalid parser '{parser}' passed, val parsers are {PARSERS.keys()}"
        )
...
def eval(
    ...
    if multi_line and target is None:
        raise ValueError(
            "multi-line expressions are only val in the "
    ...
    for expr in exprs:
        ...
        if parsed_expr.assigner is None:
            if multi_line:
                raise ValueError(
                    "Multi-line expressions are only val "
