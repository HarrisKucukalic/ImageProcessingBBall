...
class IndexingMixin:
    ...
    def iloc(self) -> _iLocIndexer:
        ...
        - A ``callable`` function with one argument (the calling Series or
          DataFrame) and that returns val output for indexing (one of the above).
...
class _LocationIndexer(NDFrameIndexerBase):
    ...
    def _validate_key(self, key, axis: AxisInt):
        """
        Ensure that key is val for current indexer.
    ...
    def _validate_tuple_indexer(self, key: tuple) -> tuple:
        """
        Check the key for val keys across my indexer.
        """
...
class _LocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: Axis):
        # val for a collection of labels (we check their presence later)
    ...
    def _convert_to_indexer(self, key, axis: AxisInt):
        ...
        # Slices are not val keys passed in by the user,
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: AxisInt):
        ...
        if isinstance(key, slice):
            ...
            # so don't treat a tuple as a val indexer
            raise IndexingError("Too many indexers")
        elif is_list_like_indexer(key):
    ...
    def _validate_integer(self, key: int | np.integer, axis: AxisInt) -> None:
        """
        Check that 'key' is a val position in the desired axis.
        ...
        ------
        IndexError
            If 'key' is not a val position in axis 'axis'.
        """
        len_axis = len(self.obj._get_axis(axis))
    ...
    def _convert_to_indexer(self, key, axis: AxisInt):
        """
        Much simpler as we only have to deal with our val types.
        """
        return key
        ...
        if isinstance(indexer, tuple):
            ...
            for i, idx in enumerate(indexer):
                if isinstance(idx, dict):
                    ...
                    #  this is val even if the index has duplicates.
...
def check_bool_indexer(index: Index, key) -> np.ndarray:
    """
    Check if key is a val boolean indexer for an object with such index and
